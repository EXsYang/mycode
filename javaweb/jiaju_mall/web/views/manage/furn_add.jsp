...
<head>
    ...
    <%--http://localhost:8080/jiaju_mall/views/manage/furn_add.jsp--%>
    <!-- 移动端适配 -->
    <%--正确书写base--%>
    ...
    <%--base 标签要写在引用资源的路径前！！
    如下写法 上面的三个link 标签没有走base 定义的路径 而是走的浏览器地址栏 资源前的路径--%>
    <%--<base href="<%=request.getContextPath() + "/"%>">--%>
...
<div class="cart-main-area pt-100px pb-100px">
    <div class="container">
        ...
        <h4 class="cart-page-title"><%=request.getAttribute("msg")%></h4> <%-- 不存在msg属性 此时显示为 null --%>
        <%--<h4 class="cart-page-title">${pageContext.request.getAttribute("msg")}</h4> --%>  <%--不存在msg属性 此时显示为 "" --%>
        <%--<h4 class="cart-page-title">${requestScope.msg}</h4> --%>  <%--不存在msg属性 此时显示为 "" --%>

        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-12">
                <%--表单提交到 furnServlet--%>
                <form action="manage/furnServlet" method="post">
                    <%--提供一个 action 用于后端反射调用对应的方法
                    addFurn 是对应action="manage/furnServlet" 里的方法名
                    --%>
                    <input type="hidden" name="action" value="addFurn">
                        <%--
                        从后端设置到session域对象中的值 在一个会话中
                        可以取出来使用
                        pageNo=${requestScope.page.pageNo}&pageSize=3
                        --%>
                        <%--在requestScope域对象中,不是请求转发过来的 而是通过一个a标签过来的
                        下面两行代码获取不到数据 ,后端获取时 是空字符串
                        --%>
...
<!--                                    <a href="#"><i class="icon-close"></i></a>-->
                            ...
                            <%--// 测试BeanUtils  反射使用的是带参构造器吗？ 还是set方法？
                                ...
                                --%>
                            <%--<tr>--%>
                            <%--<td class="product-price-cart"><input name="imgPath" style="width: 90%" type="text" value=""/></td>--%>
                            <%--</tr>--%>
                            </tbody>
                        </table>
                    </div>
