 ...
 * 2、优先级遵守: 精确路径 > 目录路径 > 扩展名路径 > /* > / > DefaultServlet
 *    即如果匹配了 "/" 会覆盖 tomcat 的 DefaultServlet， 就会返回配置了 "/" 的Servlet的返回的信息
 *    而不会返回静态资源 如OkServlet 配置了 @WebServlet(urlPatterns = "/")
 *    请求的url 为 /register.html 时 ，有匹配不上配置的其他utl-pattern 就会返回OkServlet返回的信息
 *    ，而不会返回静态资源register.html的页面内容
 *
 */
