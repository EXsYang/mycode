...
//    private static Vector<Node> nodes =null;//可以在这里定义一下
    ...
    public MyPanel(){//继承后需要自己写自己的
        ...
        switch (i1){
            case 0://继续上局游戏
                Recorder.readFirstRoundGame();
                for (int i = 0; i < Recorder.getNodes().size(); i++) {
                    EnemyTank enemyTank = new EnemyTank(Recorder.getNodes().get(i).getX(), Recorder.getNodes().get(i).getY());
                    //给每个enemyTank 设置enemyTanks!!!!
                    enemyTank.setEnemyTanks(enemyTanks);
                    //设置方向
                    enemyTank.setDirect(Recorder.getNodes().get(i).getDirect());
                    //启动敌人坦克线程，让他动起来
