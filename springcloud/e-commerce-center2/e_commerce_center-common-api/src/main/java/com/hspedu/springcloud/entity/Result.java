...
/**
 * @author yangda
 * @create 2023-12-30-16:26
 * @description: 返回结果对象，以json格式返回
 * 我们在spring+vue时用过，直接拿过来为我所用
...
public class Result<T> {
    ...
    public static <T> Result<T> success(T data){
        ...
        result.setCode("200");
        result.setMsg("success");
        // data数据已经通过构造器设置好了，不需要再进行设置一次
        // result.setData(data);
        return result;

    }

    //编写方法-返回需要的Result对象-表示成功的Result,同时可以携带数据和指定成功信息msg
