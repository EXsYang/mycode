...
public class MemberController {
    ...
    public Result save(@RequestBody Member member) {
        ...
        if (affected > 0) {//添加会员成功
            System.out.println("添加会员成功后，member对象主键id的值= " + member.getId());
    ...
    public Result getMemberById(@PathVariable("id") Long id, HttpServletRequest request) {
        ...
        if (member != null) {
            // 获取member成功
            // return Result.success("通过id获取member成功 member-service-provider-10000 " + color + " " + address, member);
            return Result.success("通过id获取member成功 member-service-provider-10000", member);
        } else {
